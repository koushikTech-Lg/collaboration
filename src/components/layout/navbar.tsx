    ...
    <div>
      ...
      {isOpen && (
        <div className="ml-4 mt-2 space-y-2">
          {item.submenu.map((subItem, index) => (
            <Link key={index} href={subItem.href || "/"} onClick={onClose}>
              <div className="px-4 py-2 text-slate-600 hover:bg-gray-50 rounded-lg transition-colors cursor-pointer md:text-sm text-xs">
                {subItem.label}
              </div>
            </Link>
          ))}
...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsScrolled(currentScrollY > 550)
  ...
  return (
    <>
      <nav
        className={`w-full fixed top-6 left-0 right-0 z-50 flex items-center justify-center px-6 transition-transform duration-300 ${
          isVisible ? "translate-y-0" : "-translate-y-[150%]"
        }`}
      >
        <Container>
          <div
            className={`w-full  backdrop-blur-xl rounded-full shadow-2xl px-8 py-1 transition-all duration-300 ${
              isScrolled ? "bg-white/90 border border-gray-200" : "bg-white/10 border border-white/20"
            }`}
          >
            <div className="flex items-center justify-between">
              {/* Logo */}
      ...
      {/* Mobile Sidebar Menu */}
      <div
        className={`fixed w-screen inset-0 z-[100] transition-opacity duration-300 ${
      ...
      >
        ...
        {/* Sidebar */}
        <div
          className={`absolute top-0 left-0 h-full w-[60%] bg-white shadow-2xl transform transition-transform duration-300 ${
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <div className="p-6 border-t">
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className=" px-8 py-3 bg-orange-500 text-white rounded-full font-semibold text-sm hover:bg-orange-600 transition-colors"
              >
                Lets's Talk
              </button>
            </div>
